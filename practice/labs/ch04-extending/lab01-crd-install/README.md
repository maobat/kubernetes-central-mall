...
In the **Central Mall**, installing a **CRD** is like adding a new official "Service Category" to the mall's legal regulations.

- **The New Regulation (CRD)** → "Starting today, the Mall officially recognizes 'ShoppingItems' as a valid legal entity with specific rules (Schema)."
...
| :--- | :--- |
| **Custom Resource Definition (CRD)** | A new legal "Charter" for the mall. |
...
1. **Definition:** Create a CRD named `shoppingitems.mall.k8s.io`.
2. **Group:** `mall.k8s.io`.
3. **Scope:** Namespaced.
...
- **CKAD Tip:** You don't need to write the CRD from scratch in the exam, but you must know how to `get` and `describe` them.
