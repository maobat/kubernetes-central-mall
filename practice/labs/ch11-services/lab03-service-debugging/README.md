# 🧪 LAB 03: Intercom Investigation ([Service](../../../../GLOSSARY.md#service) Debugging)
...
## 🛍️ Mall Analogy
In the **Central Mall**, if the Intercom ([Service](../../../../GLOSSARY.md#service)) is crackling or silent, the security guards (Administrators) must perform an **Intercom Investigation**.
...
1.  **Check the Intercom List**: List all services to find the stable Intercom IP.
    ...
    kubectl get svc
    ```
2.  **Test the Speed-Dial (DNS)**: From inside the `busybox` [pod](../../../../GLOSSARY.md#pod), try to call the [service](../../../../GLOSSARY.md#service) by its short name.
    ```bash
    nslookup hostnames
    ```
...
1.  **Inspect the Intercom Station ([Service](../../../../GLOSSARY.md#service) Spec)**: Output the [service](../../../../GLOSSARY.md#service) configuration to check for port mismatches.
    ```bash
    kubectl get service hostnames -o yaml
    ```
2.  **Verify the Connections (Endpoints)**: Check if the [Service](../../../../GLOSSARY.md#service) has actually successfully connected to any workers.
    ```bash
...
- [ ] Do the [Service](../../../../GLOSSARY.md#service) labels match the [Pod](../../../../GLOSSARY.md#pod) labels exactly?
- [ ] Is the `targetPort` in the [Service](../../../../GLOSSARY.md#service) correct for the application's port?
- [ ] Is the [Service](../../../../GLOSSARY.md#service) type correct for its intended use (ClusterIP for internal)?
