# 🎨 Kubernetes Central Mall – Comics

Visual explanations of key Kubernetes concepts using the Central Mall analogy.

---

## 🏗️ Part 1: Application Design and Build `[ 20% ]`
> **Focus:** Understanding the core blueprints of the mall, how we hire staff, organize departments, and deploy our "mannequin" images.

- **Ch. 01** – [Jobs & CronJobs](ch01-workloads/02-jobs-cronjobs/README.md)
- **Ch. 01** – [The Fixed Boutique (StatefulSets)](ch01-workloads/02-statefulsets/README.md)
- **Ch. 01** – [The Floor Security Guard (DaemonSet)](ch01-workloads/03-daemonset-floor-security/README.md)
- **Ch. 02** – [Sidecar Pattern](ch02-multi-container/01-sidecar/README.md)
- **Ch. 02** – [The Warehouse (PV/PVC)](ch02-multi-container/02-the-warehouse/README.md)
- **Ch. 03** – [The Perfect Mannequin (Image Updates)](ch03-images/03-image-updates/README.md)
- **Ch. 04** – [The Nightly Backup Permit (CRD)](ch04-extending/01-the-nightly-backup-permit/README.md)
- **Ch. 04** – [The Manager with the Manual (Operators)](ch04-extending/04-operators/README.md)

---

## 🔐 Part 2: Application Environment, Configuration and Security `[ 25% ]`
> **Focus:** Managing the mall's vault, setting employee conduct rules, and balancing the utility budget.

- **Ch. 05** – [The Rulebook & The Uniform (ConfigMap)](ch05-config-secrets/01-configmap/README.md)
- **Ch. 05** – [High-Security Vault (Secrets)](ch05-config-secrets/01-secrets-injection/README.md)
- **Ch. 06** – [Worker Safety (SecurityContext)](ch06-safety/02-worker-safety/README.md)
- **Ch. 07** – [The Secure Badge (RBAC)](ch07-identity/01-the-secure-badge/README.md)
- **Ch. 07** – [The HR Manual (Role)](ch07-identity/02-the-hr-manual/README.md)
- **Ch. 07** – [The Entry Permit Office (Admission Control)](ch07-identity/03-admission-control/README.md)
- **Ch. 08** – [The Resource Hog (Requests/Limits)](ch08-resources/01-the-resource-hog/README.md)

---

## 🚀 Part 3: Application Deployment `[ 20% ]`
> **Focus:** Grand openings, logistics pipelines, and managing change without disappointing customers.

- **Ch. 09** – [The Canary Side Entrance](ch09-launch/01-canary-nodeport/README.md)
- **Ch. 09** – [The Sign Swap (Blue/Green)](ch09-launch/01-blue-green-sign-swap/README.md)
- **Ch. 10** – [The Logistics Chain (Helm & Kustomize)](ch10-logistics/02-the-logistics-chain/README.md)

---

## 🌐 Part 4: Services and Networking `[ 20% ]`
> **Focus:** Intercoms, signage, and managing the grand entrances of the mall.

- **Ch. 11** – [The Internal Intercom (ClusterIP)](ch11-services/01-internal-intercom/README.md)
- **Ch. 11** – [The NodePort Traffic Adventure](ch11-services/02-cross-namespace/README.md)
- **Ch. 12** – [Virtual Host Gateway Show](ch12-ingress/01-virtual-host/README.md)
- **Ch. 12** – [The Lost Gateway](ch12-ingress/01-the-lost-gateway/README.md)
- **Ch. 13** – [The Locked Corridor (Network Policies)](ch13-networking/01-locked-corridors/README.md)

---

## 🩺 Part 5: Application Observability and Maintenance `[ 15% ]`
> **Focus:** Health inspections, incident investigations, and the mall control room.

- **Ch. 14** – [The Health Inspector - Liveness/Readiness](ch14-probes/01-the-health-inspector/README.md)
- **Ch. 15** – [The Broken Shop (Troubleshooting)](ch15-debugging/03-the-broken-shop/README.md)

---
[Back to Main Index](../../README.md)
