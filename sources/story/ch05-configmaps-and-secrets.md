...
Instead, the Owner uses a **Public Bulletin Board**. This is the **[ConfigMap](../../GLOSSARY.md#configmap)**.

1. The Owner writes the current prices on a sheet of paper.
2. They hand it to the **Management Office ([API Server](../../GLOSSARY.md#api-server))**, which pins it to the mall's digital bulletin board (stored in the Ledger).
3. When the **Worker ([Pod](../../GLOSSARY.md#pod))** arrives for their shift, they "mount" a copy of that Price List directly onto their desk. 
...
For this, the Mall provides the **Secure Locked Box**. This is the **[Secret](../../GLOSSARY.md#secret)**.

Secrets work just like ConfigMaps, but with extra layers of caution:
- **The Box is Opaque:** To a casual observer walking by the Management Office, the content of the [Secret](../../GLOSSARY.md#secret) is obscured (Base64 encoded). 
- **Restricted Access:** Only workers with the correct clearance ([RBAC](../../GLOSSARY.md#rbac)) can even see that the box exists.
...
Whether those drawers are filled with **Environment Variables** (loose papers) or **Mounted Volumes** (a bound folder), the worker's [job](../../GLOSSARY.md#job) remains the same. This decoupling allows the Mall Owner to change the "business logic" (the prices and keys) without ever disrupting the "workforce" (the Pods).
